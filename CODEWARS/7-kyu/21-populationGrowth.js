...
function nbYear(p0, percent, aug, p) {
  let years = 0;
  let populationPerYear;

  while (p > p0) {        //! leer enunciado, te está hablando de redondear... 
    populationPerYear = Math.floor(p0 + (p0 * (percent / 100)) + aug);
    p0 = populationPerYear
    years++;           //! si es +1, está expresión es buena idea usarla ++
  }
  return years;
}
